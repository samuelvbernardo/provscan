...
class ExamSerializer(serializers.ModelSerializer):
    ...
    def validate_questions_count(self, value):
        if value < 8 or value > 30:
            raise serializers.ValidationError("A quantidade de questões deve estar entre 8 e 30.")
        return value

    def validate_options_count(self, value):
        if value not in [4, 5]:
            raise serializers.ValidationError("A quantidade de alternativas deve ser 4 ou 5.")
        return value
    ...
    def validate(self, attrs):
        ...
        if not answer_key:
            raise serializers.ValidationError({"answer_key": "O gabarito da prova é obrigatório."})
        ...
        if len(answer_key) != questions_count:
            raise serializers.ValidationError(
                {"answer_key": (f"O gabarito deve ter exatamente {questions_count} respostas.")}
            )
...
class ScanResultSerializer(serializers.ModelSerializer):
    ...
    def get_warnings(self, obj):
        ...
        if not obj.student_number or "?" in obj.student_number:
            warnings.append("Não foi possível identificar corretamente o número do aluno.")
        ...
        if obj.answers:
            blank_or_invalid = [
                index + 1 for index, answer in enumerate(obj.answers) if answer is None
            ]
            if blank_or_invalid:
                warnings.append(

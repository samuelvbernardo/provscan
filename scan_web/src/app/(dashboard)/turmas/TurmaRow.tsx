...
import { updateClassGroup, deleteClassGroup } from '@/app/actions/turmas'
import type { ClassGroup, FormState } from '@/lib/definitions'
...
export default function TurmaRow({ turma }: { turma: ClassGroup }) {
  const [editing, setEditing] = useState(false)
  const [state, action, pending] = useActionState(
    async (prevState: FormState, formData: FormData): Promise<FormState> => {
      const nextState = await updateClassGroup(prevState, formData)
      if (nextState?.ok) {
        setEditing(false)
      }
      return nextState
    ...
    return (
      <tr className="bg-indigo-50/60">
        <td colSpan={5} className="px-5 py-4">
          <p className="text-xs font-semibold text-indigo-700 mb-3">Editar turma</p>
          <form action={action} className="flex flex-wrap items-end gap-3">
            ...
            <div className="w-36">
              ...
              <input
                name="school_year"
                defaultValue={turma.school_year ?? ''}
      ...
      <td className="px-5 py-3.5">
        <span
          className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
            turma.is_active ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500'
          }`}
        >
